...
import 'package:flutter_demo/helper/cons/cons.dart';
...
//构造方法中刷新小球
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.translate(16 * _radius, 0);
    renderDigit(_now.hour % 10, canvas);

    canvas.translate(14 * _radius, 0);
    renderDigit(10, canvas);

    canvas.translate(8 * _radius, 0);
  ...
  void renderDigit(int num, Canvas canvas) {
    ...
    for (int i = 0; i < digit[num].length; i++) {
      for (int j = 0; j < digit[num][j].length; j++) {
        if (digit[num][i][j] == 1) {
          canvas.save();
          double rX = j * 2 * (_radius ) + (_radius + 1); //第(i，j)个点圆心横坐标
          double rY = i * 2 * (_radius ) + (_radius + 1); //第(i，j)个点圆心纵坐标
